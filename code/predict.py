...
test_path = glob.glob('./test_set_B/*.jpg')
test_path.sort()
...
for fold_num in range(1, N_SPLITS + 1):
    ...
    model.load_state_dict(torch.load(f'best_convnext_512_fold_{fold_num}.pth'))
...
# 保存为CSV文件
results.to_csv('submission_ema.csv', index=False)
print("预测结果已保存为 submission_ema.csv")
